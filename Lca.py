...
# Load data (assumed CSV, replace with your path)
df = pd.read_csv(r"C:\Users\ASUS\Desktop\MidProject\LCA_Data_For_analysis.csv", low_memory=False)

# Ensure the 'RECEIVED_DATE' is a datetime object
df['RECEIVED_DATE'] = pd.to_datetime(df['RECEIVED_DATE'], errors='coerce')

# --- Year Filter in Sidebar ---
# نقوم باستخراج السنوات من عمود RECEIVED_DATE
min_year = max(2019, int(df['RECEIVED_DATE'].dt.year.min()))  # نبدأ من 2019 على الأقل
max_year = int(df['RECEIVED_DATE'].dt.year.max())
# نضيف خيار "All Years" في حال رغب المستخدم في إظهار كافة البيانات
...
def page1():
    ...
    with col2:
        ...
        # نضيف عمود السنة من RECEIVED_DATE في البيانات المفلترة
...
def page4():
    ...
    fig = px.histogram(
        ...
        title='Top NAICS Codes Distribution with Descriptions'
    )
    st.plotly_chart(fig)
